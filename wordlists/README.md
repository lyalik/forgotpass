...
В репозиторий включён `rockyou.txt` — топ-75% самых частых паролей из базы RockYou
(~59 000 паролей, 478 KB). Этого достаточно для большинства забытых паролей и
работает **полностью офлайн** — без скачивания из интернета.

## Полная версия (опционально)

Для сложных случаев можно скачать полную версию rockyou.txt (14M паролей, 133MB):

```bash
./install.sh --full-rockyou
```

Полная версия заменит встроенную в `wordlists/rockyou.txt`.
...
3. `~/rockyou.txt`
4. `./wordlists/rockyou.txt` (в каталоге проекта — встроенный)
